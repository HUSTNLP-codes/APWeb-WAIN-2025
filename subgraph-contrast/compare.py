...
parser.add_argument('--alpha', type=float, default=0.9, help='how much the first view involves')
parser.add_argument('--beta', type=float, default=0.3, help='how much context-level involves')
...
if __name__ == '__main__':
    ...
    for run in range(args.runs):
        ...
        model_twin = Model(ft_size, args.embedding_dim, 'prelu', args.negsamp_ratio_patch, args.negsamp_ratio_context,
                      args.readout).to(device)
        optimiser = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        optimiser_twin = torch.optim.Adam(model_twin.parameters(), lr=args.lr, weight_decay=args.weight_decay)
        ...
        for epoch in range(args.num_epoch):
            ...
            for batch_idx in range(batch_num):
                ...
                logits_3, logits_4, subgraph_embed_twin, node_embed_twin = model_twin(bf, ba)
                logits_3_hat, logits_4_hat, subgraph_embed_hat_twin, node_embed_hat_twin = model_twin(bf, ba_hat)
                ...
                loss_all_3 = b_xent_context(logits_3, lbl_context)
                loss_all_3_hat = b_xent_context(logits_3_hat, lbl_context)
                loss_3 = torch.mean(loss_all_3)
                loss_3_hat = torch.mean(loss_all_3_hat)

                loss_all_4 = b_xent_patch(logits_4, lbl_patch)
                loss_all_4_hat = b_xent_patch(logits_4_hat, lbl_patch)
                loss_4 = torch.mean(loss_all_4)
                loss_4_hat = torch.mean(loss_all_4_hat)
                ...
                loss_3 = args.alpha * loss_3 + (1 - args.alpha) * loss_3_hat  # node-subgraph contrast loss
                loss_4 = args.alpha * loss_4 + (1 - args.alpha) * loss_4_hat  # node-node contrast loss
                # loss = args.beta * loss_1 + (1 - args.beta) * loss_2 + 0.1 * NCE_loss #total loss
                #loss = loss_3 + loss_4
                loss = loss_1 + loss_2 + loss_3 + loss_4 
                # loss = 0.5 * loss_1 + (1 - 0.5) * loss_2 + 0.5 * NCE_loss#total loss
            ...
            if mean_loss < best:
                ...
                torch.save(model_twin.state_dict(), '{}.pkl'.format(args.dataset + '_twin'))
            else:
                cnt_wait += 1
        ...
        model_twin.load_state_dict(torch.load('{}.pkl'.format(args.dataset + '_twin')))
            ...
            for round in range(args.auc_test_rounds):
                ...
                for batch_idx in range(batch_num):
                    ...
                    with torch.no_grad():
                        ...
                        test_logits_3, test_logits_4, _, _ = model_twin(bf, ba)
                        test_logits_3_hat, test_logits_4_hat, _, _ = model_twin(bf, ba_hat)
                        test_logits_3 = torch.sigmoid(torch.squeeze(test_logits_3))
                        test_logits_4 = torch.sigmoid(torch.squeeze(test_logits_4))
                        test_logits_3_hat = torch.sigmoid(torch.squeeze(test_logits_3_hat))
                        test_logits_4_hat = torch.sigmoid(torch.squeeze(test_logits_4_hat))
                        ...
                        ano_score_2_hat = - (
                                ...
                                cur_batch_size, args.negsamp_ratio_patch), dim=1)).cpu().numpy()
                        ano_score_3 = - (
                                    test_logits_3[:cur_batch_size] - torch.mean(test_logits_3[cur_batch_size:].view(
                                cur_batch_size, args.negsamp_ratio_context), dim=1)).cpu().numpy()
                        ano_score_3_hat = - (
                                test_logits_3_hat[:cur_batch_size] - torch.mean(test_logits_3_hat[cur_batch_size:].view(
                            cur_batch_size, args.negsamp_ratio_context), dim=1)).cpu().numpy()
                        ano_score_4 = - (
                                    test_logits_4[:cur_batch_size] - torch.mean(test_logits_4[cur_batch_size:].view(
                                cur_batch_size, args.negsamp_ratio_patch), dim=1)).cpu().numpy()
                        ano_score_4_hat = - (
                                test_logits_4_hat[:cur_batch_size] - torch.mean(test_logits_4_hat[cur_batch_size:].view(
                            cur_batch_size, args.negsamp_ratio_patch), dim=1)).cpu().numpy()
                        ...
                        #ano_score =  args.beta * (args.alpha * ano_score_1 + (1 - args.alpha) * ano_score_1_hat) + \
                        #           (1 - args.beta) * (args.alpha * ano_score_2 + (1 - args.alpha) * ano_score_2_hat)
                        ano_score =  args.alpha * ano_score_3 + (1 - args.alpha) * ano_score_3_hat + \
                                    args.alpha * ano_score_4 + (1 - args.alpha) * ano_score_4_hat + \
                                    args.alpha * ano_score_3 + (1 - args.alpha) * ano_score_3_hat + \
                                   args.alpha * ano_score_4 + (1 - args.alpha) * ano_score_4_hat
                        #ano_score =  args.beta * (args.alpha * ano_score_1 + (1 - args.alpha) * ano_score_1_hat) + \
                        #           (1 - args.beta) * (args.alpha * ano_score_2 + (1 - args.alpha) * ano_score_2_hat) + \
                        #          args.beta * (args.alpha * ano_score_3 + (1 - args.alpha) * ano_score_3_hat) + \
                        #          (1 - args.beta) * (args.alpha * ano_score_4 + (1 - args.alpha) * ano_score_4_hat)
                                  

                            # ano_score = ano_score_1 + ano_score_1_hat +  ano_score_2 +  ano_score_2_hat
