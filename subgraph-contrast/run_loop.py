...
if __name__ == '__main__':
    ...
    para_list = [0.1, 0.3, 0.5, 0.7, 0.9]
    for alpha in para_list:
        for beta in para_list:
            for gamma in para_list:
                ...
                for run in range(args.runs):
                    ...
                    model = Model(ft_size, args.embedding_dim, 'prelu', args.negsamp_ratio_patch,
                    ...
                    b_xent_context = nn.BCEWithLogitsLoss(reduction='none',
                                                          pos_weight=torch.tensor([args.negsamp_ratio_context]).to(
                                                              device))
                    ...
                    for epoch in range(args.num_epoch):
                        ...
                        for batch_idx in range(batch_num):
                            ...
                            lbl_patch = torch.unsqueeze(torch.cat(
                                (torch.ones(cur_batch_size), torch.zeros(cur_batch_size * args.negsamp_ratio_patch))),
                                1).to(device)

                            lbl_context = torch.unsqueeze(torch.cat(
                                (torch.ones(cur_batch_size), torch.zeros(cur_batch_size * args.negsamp_ratio_context))),
                                1).to(device)
                            ...
                            sim_row_sum = sim_matrix_one_exp[:, nega_list] + sim_matrix_two_exp[:,
                                                                             nega_list] + sim_matrix_three_exp[:,
                                                                                          nega_list]
                            ...
                            loss_1 = alpha * loss_1 + (1 - alpha) * loss_1_hat  # node-subgraph contrast loss
                            loss_2 = alpha * loss_2 + (1 - alpha) * loss_2_hat  # node-node contrast loss
                            loss = beta * loss_1 + (1 - beta) * loss_2 + gamma * NCE_loss  # total loss
                        ...
                        for round in range(args.auc_test_rounds):
                            ...
                            for batch_idx in range(batch_num):
                                ...
                                with torch.no_grad():
                                    ...
                                    ano_score_1 = - (test_logits_1[:cur_batch_size] - torch.mean(
                                        test_logits_1[cur_batch_size:].view(
                                            cur_batch_size, args.negsamp_ratio_context), dim=1)).cpu().numpy()
                                    ano_score_1_hat = - (
                                            test_logits_1_hat[:cur_batch_size] - torch.mean(
                                        test_logits_1_hat[cur_batch_size:].view(
                                            cur_batch_size, args.negsamp_ratio_context), dim=1)).cpu().numpy()
                                    ano_score_2 = - (test_logits_2[:cur_batch_size] - torch.mean(
                                        test_logits_2[cur_batch_size:].view(
                                            cur_batch_size, args.negsamp_ratio_patch), dim=1)).cpu().numpy()
                                    ano_score_2_hat = - (
                                            test_logits_2_hat[:cur_batch_size] - torch.mean(
                                        test_logits_2_hat[cur_batch_size:].view(
                                            cur_batch_size, args.negsamp_ratio_patch), dim=1)).cpu().numpy()
                                    ano_score = beta * (
                                                alpha * ano_score_1 + (1 - alpha) * ano_score_1_hat) + \
                                                (1 - beta) * (alpha * ano_score_2 + (1 - alpha) * ano_score_2_hat)
